...
object Assertion {
  ...
  def fromFunction[A](name: String)(assertion: A => AssertResult): zio.test.Assertion[A] =
    assertionDirect(name)() { a => assertion(a) }

  def map[A]: MapAssert[A] = new MapAssert

  class MapAssert[A] {
    def apply[B](name: String, f: A => B)(assertion0: Assertion[B]): Assertion[A] =
  ...
  class MapAssertN[A] {
    def apply[B](name: String, f: A => B)(assertion0: Assertion[List[B]]): Assertion[List[A]] =
...
object TracingAssertion {
  ...
  def containsHeader(key: String, value: String): Assertion[Response[Either[String, String]]] =
    hasField("header", _.request.header(key), equalTo(Some(value)))

  def hasAttribute[A](key: String, value: A): Assertion[CompletedSpan] =
    hasField(key, _.attributes.get(key).map(_.value.value), equalTo(Some(value)))
