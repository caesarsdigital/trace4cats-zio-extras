...
final class ZSpan(span: Span[Task]) {
  ...
  def put(key: String, value: AttributeValue): UIO[Unit] =
    span.put(key, value).ignore
  def putAll(fields: (String, AttributeValue)*): UIO[Unit] =
    span.putAll(fields*).ignore
  def putAll(fields: Map[String, AttributeValue]): UIO[Unit] =
    span.putAll(fields).ignore
  def setStatus(spanStatus: SpanStatus): UIO[Unit] =
    span.setStatus(spanStatus).ignore
  def addLink(link: Link): UIO[Unit] = span.addLink(link).ignore
  def addLinks(links: NonEmptyList[Link]): UIO[Unit] =
    span.addLinks(links).ignore
