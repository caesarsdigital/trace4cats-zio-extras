...
object TracingUtils {
  private val sampler: ULayer[Has[SpanSampler[Task]]] =
    ...
    AsyncHttpClient
      ...
      .map(
        Logger(
          logHeaders = true,
          logBody = false,
          logAction = Some(s => Task(org.log4s.getLogger.debug(s))),
        ),
      )
      .toManagedZIO
      .orDie
  ...
  final case class NewrelicCompleterConfig(
  ...
  val entryPointLayer: URLayer[Has[
    SpanSampler[Task],
  ] & Has[SpanCompleter[Task]], Has[EntryPoint[Task]]] =
    (for {
  ...
  val newRelicEntryPoint: URLayer[Has[NewrelicCompleterConfig], Has[EntryPoint[Task]]] =
    sampler ++ (ZLayer.service[
      NewrelicCompleterConfig,
    ] >>> newRelicCompleter) >>> entryPointLayer
}
