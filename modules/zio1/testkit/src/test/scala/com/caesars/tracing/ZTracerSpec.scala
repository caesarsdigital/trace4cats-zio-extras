    ...
    suite("ZTracer")(
      testM("starts out with no span") {
        ZIO
          .service[ZTracer]
          .flatMap(_.context)
          .map(context => assertTrue(context == SpanContext.invalid))
      },
      testM("can create a span and add attributes to it") {
        spanSpec()(
          _.putAll(("fieldName", StringValue("fieldValue"))),
          queue => queue.get.map(_.head),
        ).map { cs =>
          assertTrue(
      ...
      testM("can create a span and set its status") {
        spanSpec()(
          _.setStatus(SpanStatus.Aborted),
          completer => completer.get.map(_.head),
        ).map { cs =>
          assertTrue(
      ...
      testM("can simultaneously create multiple roots") {
        ...
        for {
          ...
          result
            .flatMap(span => evalAttributes(span.attributes).get("a"))
            .toSet ==
      ...
      testM("can create nested spans") {
        for {
          tracer <- ZIO.service[ZTracer]
          _ <- tracer.span("parent")(
            _.child("child", SpanKind.Internal).use(_ => ZIO.unit),
          )
          completer <- ZIO.service[RefSpanCompleter[Task]]
          result    <- completer.get
          parent = result
