...
object TestWebServer {
  ...
  val randomPort: ULayer[Has[Port]] =
    Random.live >>> ZIO
      ...
      .flatMap(_.nextIntBetween(1024, 65535))
      .map(Port(_))
      .toLayer
  ...
  def trace[A](
    ...
    Port,
  ] & Has[ZIOSttpClient] & Has[CompletedSpanQueue], (List[CompletedSpan], A)] = {
    for {
      ...
      ZIO
        ...
        .map(cq =>
          new RefSpanCompleter(
            TraceProcess("ZTracerImplementationSpec"),
            cq.ref,
          ),
        )
        .toLayer
  ...
  val mkTracedClient: URLayer[Has[CompletedSpanQueue] & Has[ZIOSttpClient], Has[
    ZIOSttpClient,
  ]] =
    (ZLayer.service[ZIOSttpClient] ++ mkZTracer ++ ZLayer.succeed(
      TracedBackendConfig(),
    )) >>> TracedBackend.layer

  private val serverRequirements =
    ZLayer.service[ZIOSttpClient] ++ ZLayer.service[Port] ++ EventLoopGroup
      .auto() ++ ServerChannelFactory.auto
  ...
  val mkTracedServer = {
    val startServer = (
      for {
        ...
        _ <- (Server.app(TracedHttp(tracer)(server(client, port))) ++ Server
          .port(port.value)).make
      } yield ()
    ).toLayer
